...
def preprocess_for_model(pil_image, is_front_camera=False, save_debug=True):
    ...
    Reproduce prediction.py EXACTLY:
      1. Scale large images PROPORTIONALLY (preserve aspect ratio)
    ...
    Returns (preprocessed_batch, hand_detected_bool, debug_info)
    """
    ...
    # ═══ FIX: Scale large images PROPORTIONALLY ═══
    # prediction.py captures at 640×480 from webcam.
    # Mobile sends much higher res — scale down but KEEP aspect ratio.
    ...
    if max(h_orig, w_orig) > max_dim:
        ...
        debug_info['scaled_to'] = f'{w_orig}x{h_orig} (no scale)'
    ...
    if bbox is None:
        ...
        return np.expand_dims(img_resized / 255.0, axis=0), False, debug_info
    ...
    return np.expand_dims(final, axis=0), True, debug_info
...
def predict_sign():
    ...
    try:
        ...
        batch, hand_detected, debug_info = preprocess_for_model(
        ...
        # Same concept as prediction.py's real-time display where
        # the user sees a stable label because it runs continuously.
        ...
        if hand_detected and len(prediction_buffer) >= 2:
            # Use majority vote from buffer for stable prediction
            labels_in_buffer = [lb for lb, _ in prediction_buffer]
            label_counts = Counter(labels_in_buffer)
            majority_label, majority_count = label_counts.most_common(1)[0]

            # Only use majority vote if it appears in at least 40% of buffer
            if majority_count >= max(2, len(prediction_buffer) * 0.4):
                label = majority_label
                # Average confidence of the majority label
        ...
        if expected:
            ...
            status = "predicted"
            is_correct = True  # no expected label, treat as correct
    ...
    try:
        ...
        if predicted and expected:
            if predicted == expected:
                status = 'correct'
                feedback = 'Great job! You signed it correctly!'
                feedback_level = 'excellent'
            else:
                status = 'incorrect'
                feedback = f'Detected "{predicted}" but expected "{expected}". Keep practising the motion.'
                feedback_level = 'poor'
        elif predicted:
            status = 'predicted'
